...
<template>
  <span class="inline-flex">
    <button
      ref="triggerRef"
      type="button"
      class="center-row h-[16px] w-[16px] cursor-help rounded-full text-ink-muted transition-colors duration-150 hover:text-ink-secondary"
    ...
    <Teleport to="body">
      <div
        v-if="isOpen"
        ref="tooltipRef"
        class="fixed z-[10000] flex max-h-[320px] max-w-[420px] flex-col overflow-hidden rounded-lg border border-line-strong bg-surface-input px-3 py-2 text-left text-[12px] leading-relaxed text-ink-secondary shadow-pop"
        :style="tooltipStyle"
        @mouseenter="showTooltip"
        @mouseleave="scheduleHideTooltip"
      >
        <div v-if="showCopyButton" class="mb-2 flex shrink-0 justify-end">
          <button
            type="button"
            class="center-row gap-1 rounded-md border border-line-strong bg-surface-hover px-2 py-1 text-[11px] text-ink-secondary transition-colors duration-150 hover:border-line hover:bg-line"
            @click="handleCopy"
          >
