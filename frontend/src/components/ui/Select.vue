...
<template>
  <div ref="rootRef" class="relative">
    <button
      ...
      type="button"
      :disabled="disabled"
      class="flex h-9 items-center rounded-md bg-surface-input px-3 text-left text-sm text-ink-primary outline-none transition-colors disabled:cursor-not-allowed disabled:opacity-60"
      :class="[
        border
          ? 'w-full justify-between gap-2 border border-line-strong focus:border-brand-500'
          : 'w-auto justify-start gap-2 border border-transparent focus-visible:ring-2 focus-visible:ring-brand-500/40',
        buttonClass,
      ]"
    ...
    >
      <span
        ...
        :class="[
          ...
          selectedOption
            ? (border ? 'text-ink-primary' : 'text-current')
            : 'text-ink-muted',
        ]"
      >
      ...
      <span
        ...
        :class="[border ? 'text-ink-muted' : 'text-current', isOpen ? 'rotate-180' : '']"
      >
  ...
  <Teleport to="body">
    ...
    >
      <div
        v-if="isOpen"
        ref="menuRef"
        class="fixed z-[999] overflow-hidden rounded-lg border border-line-strong bg-surface-input p-1 shadow-pop"
        :class="menuClass"
        :style="menuStyle"
      >
