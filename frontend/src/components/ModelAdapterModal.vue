    ...
    <Transition name="modal-mask">
      ...
      >
        <Transition name="modal-content">
          <div
            v-show="visible"
            class="relative z-10 w-full max-w-[560px] overflow-hidden rounded-lg border border-line bg-surface-card p-5 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.6)]"
            @click.stop
          >
            <div>
              <h3 class="mb-4 text-base font-medium text-ink-primary">{{ title }}</h3>

              <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                <label class="flex flex-col gap-1">
                  <span class="text-sm text-ink-secondary">显示名称</span>
                  <input
                    v-model="draft.displayName"
                    type="text"
                    class="h-9 rounded-[6px] border border-line-strong bg-surface-input px-3 text-sm text-ink-primary outline-none focus:border-brand-500"
                  />
                </label>

                <label class="flex flex-col gap-1">
                  <span class="text-sm text-ink-secondary">ModelID</span>
                  <input
                ...
                <label class="flex flex-col gap-1">
                  <span class="text-sm text-ink-secondary">API Key</span>
                  <input
              ...
              <label class="mt-3 flex flex-col gap-1">
                <span class="text-sm text-ink-secondary">baseURL</span>
                <input
              ...
              <label class="mt-3 flex flex-col gap-1">
                <span class="text-sm text-ink-secondary">context_window_tokens</span>
                <input
              ...
              <div v-if="draft.type === 'openai'" class="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
                <label class="flex flex-col gap-1">
                  <span class="text-sm text-ink-secondary">reasoning_effort</span>
                  <Select
                ...
                <label class="flex flex-col gap-1">
                  <span class="text-sm text-ink-secondary">max token</span>
                  <input
                ...
                <label class="flex flex-col gap-1">
                  <span class="text-sm text-ink-secondary">endpoint</span>
                  <Select
              ...
              <div v-if="draft.type === 'openai'" class="mt-3 rounded-[8px] border border-line bg-surface-hover p-3">
                <div class="flex items-center justify-between gap-3">
                  <span class="flex items-center gap-1.5 text-sm text-ink-secondary">
                ...
                <textarea
                  ...
                  rows="5"
                  spellcheck="false"
                  class="mt-3 min-h-[120px] w-full resize-none rounded-[6px] border border-line-strong bg-surface-input px-3 py-2 font-mono text-xs text-ink-primary outline-none focus:border-brand-500"
                />
              </div>

              <div v-if="draft.type === 'anthropic'" class="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
                <label class="flex flex-col gap-1">
                  <span class="text-sm text-ink-secondary">max_tokens</span>
                  <input
                ...
                <label class="flex flex-col gap-1">
                  <span class="text-sm text-ink-secondary">thinking effort</span>
                  <Select
              ...
              <label class="mt-3 flex flex-col gap-1">
                <span class="text-sm text-ink-secondary">tooltipData</span>
                <textarea
                  v-model="draft.tooltipData"
                  rows="5"
                  class="min-h-[120px] resize-none rounded-[6px] border border-line-strong bg-surface-input px-3 py-2 text-sm text-ink-primary outline-none focus:border-brand-500"
                />
              </label>

              <div
                v-if="errorMessage"
                class="mt-4 rounded-[8px] border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-400"
              >
                {{ errorMessage }}
              </div>
